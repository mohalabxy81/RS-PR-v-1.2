...
let ReportsService = class ReportsService {
    ...
    async getLeadReport(tenantId, query) {
        const { startDate, endDate, branchId } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
        const baseWhere = { tenantId, ...(branchId && { branchId }), ...dateFilter };
        const [byStatus, bySource, byAgent, totalLeads, wonLeads, lostLeads,] = await Promise.all([
    ...
    async getDealReport(tenantId, query) {
        const { startDate, endDate, branchId } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
        const baseWhere = { tenantId, ...dateFilter };
        const [byStage, totalRevenue, forecastRevenue, totalDeals, closedWon, closedLost, byAgent,] = await Promise.all([
    ...
    async getPropertyReport(tenantId, query) {
    ...
    async getAgentReport(tenantId, query) {
        const { startDate, endDate, branchId } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
    ...
    async getAppointmentReport(tenantId, query) {
        const { startDate, endDate } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
        const baseWhere = { tenantId, ...(dateFilter.createdAt ? { startTime: dateFilter.createdAt } : {}) };
    ...
    async exportLeadsCsv(tenantId, query) {
        const { startDate, endDate } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
    ...
    async exportDealsCsv(tenantId, query) {
        const { startDate, endDate } = query;
        const dateFilter = this.buildDateFilter(startDate, endDate);
